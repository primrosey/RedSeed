...
func add_shop_modifier(shop: Shop) -> void:
	...
	var coupons_modifier_value := shop_cost_modifier.get_value(modifier_id)
	
	if not coupons_modifier_value:
		coupons_modifier_value = ModifierValue.create_new_modifier(modifier_id, ModifierValue.Type.PERCENT_BASED)
