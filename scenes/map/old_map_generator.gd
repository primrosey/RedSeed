...
const CAMPFIRE_ROOM_WEIGHT := 0.0 #4.0
...
func _would_cross_existing_path(i: int, j: int, room: Room) -> bool:
	...
	# if j == 0 theres no left
	if j > 0:
		left_neighbor = map_data[i][j - 1]
	# if j == MAP_WIDTH -1 theres no right
	if j < MAP_WIDTH - 1:
	...
	# can't cross in left dir if left neighbot goes right
...
func _set_room_randomly(room_to_set: Room) -> void:
	...
	while campifre_below_4 or consecutive_campfire or consecutive_shop or campfire_on_13:
		...
		campifre_below_4 = is_campfire and room_to_set.row < 3
