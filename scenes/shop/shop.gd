...
const SHOP_RELIC_SCENE = preload("res://scenes/shop/shop_relic.tscn")
...
func _generate_shop_cards() -> void:
	...
	var avaliable_cards := char_stats.draftable_cards.duplicate_cards()
	RNG.array_shuffle(avaliable_cards)
	shop_card_array = avaliable_cards.slice(0, 3)
	
	for card: Card in shop_card_array:
		var new_shop_card := SHOP_CARD_SCENE.instantiate() as ShopCard
		cards.add_child(new_shop_card)
		new_shop_card.card = card
		new_shop_card.current_card_menu_ui.tooltip_requested.connect(card_tooltip_popup.show_tooltip)
...
func _generate_shop_relics() -> void:
	var shop_relic_array: Array[Relic] = []
	var avaliable_relics := shop_relics.filter(
		func(relic: Relic):
	...
	RNG.array_shuffle(avaliable_relics)
	shop_relic_array = avaliable_relics.slice(0, 3)
	
	for relic: Relic in shop_relic_array:
		var new_shop_relic := SHOP_RELIC_SCENE.instantiate() as ShopRelic
