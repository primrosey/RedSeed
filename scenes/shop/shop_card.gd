...
@onready var gold_cost := RNG.instance.randi_range(100,300) # maybe minimum cost on a per-card/rarity basis

var current_card_menu_ui: CardMenuUI
...
func set_card(new_card: Card) -> void:
	...
	var new_card_menu_ui := CARD_MENU_UI_SCENE.instantiate() as CardMenuUI
	card_container.add_child(new_card_menu_ui)
	new_card_menu_ui.card = card
	current_card_menu_ui = new_card_menu_ui

func _on_buy_button_pressed() -> void:
	Events.shop_card_bought.emit(card, gold_cost)
	# delete children but leave the vboxcontainer so the parent hbox leaves the blank space
