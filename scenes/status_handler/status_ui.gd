...
func set_status(new_status: Status) -> void:
	...
	stacks.visible = status.stack_type == Status.StackType.INTENSITY or status.stack_type == Status.StackType.BOTH
...
func _on_status_changed() -> void:
	...
	if status.stack_type == Status.StackType.BOTH and status.stacks <= 0:
		queue_free()
