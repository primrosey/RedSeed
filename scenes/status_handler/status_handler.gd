...
func apply_statuses_by_type(type: Status.Type) -> void:
	...
	var status_queue: Array[Status] = _get_all_status().filter(
		func(status: Status):
...
func add_status(status: Status) -> void:
	...
	#If it's unique and we already have it, return
	...
	if status.stack_type == Status.StackType.INTENSITY or status.stack_type == Status.StackType.BOTH:
...
func _on_gui_input(event: InputEvent) -> void:
	if event.is_action_pressed("left_mouse"):
		Events.status_tooltip_requested.emit(_get_all_status())
